jQuery.fn.softwareNumPad = function() {
    var softwareNumPad = jQuery(".software-numpad"),
    ...
    if (!softwareNumPad.length) {
        softwareNumPad = jQuery("<div>")
            .addClass("software-numpad")
        ...
        jQuery.each(keys, function(i, row) {
            var rowDiv = jQuery("<div>")
                .attr("class", "row")
                .appendTo(softwareNumPad);

            jQuery.each(row, function(j, key) {
                var keyClass = "key key-" + ({ ".": "dot" }[ key ] || key);
                var corner = corners[ key ] || "";
                var keySpan = jQuery("<span class='ui-btn " + corner +
                    " ui-btn-up-c'><span class='ui-btn-inner " + corner + "'>" +
                     "<span class='ui-btn-text'>" +
                     (key === "bs" ? "&nbsp;" : key) + "</span>" +
                    (key === "bs" ? "<span class='ui-icon " +
                        "ui-icon-back ui-icon-shadow'></span>" : "") +
                        "</span></span>")
                    .addClass(keyClass)
                    .appendTo(rowDiv);
